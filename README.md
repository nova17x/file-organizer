...
### GUI機能
- **PyQt6ベースのモダンUI**: 洗練されたグラフィカルインターフェース
- **テーマ切り替え対応**: ダーク/ライトテーマを選択可能（設定メニュー）
- **進捗表示**: プログレスバーで処理状況を可視化
- **カスタムルール設定画面**: 柔軟なルール作成が可能
- **非同期処理**: 長時間の操作でもUIがフリーズしない
...
**注意**: PyQt6のインストールには少し時間がかかる場合があります。
...
PyQt6が正しくインストールできない場合:

```bash
pip install --upgrade PyQt6
```

システムによっては追加の依存関係が必要な場合があります。
...
**Q: 整理前にファイルを確認できますか？**
A: はい、「プレビュー」ボタンで整理内容を事前に確認できます。フィルター機能も利用可能です。
...
**Q: 大量のファイルを処理できますか？**
A: はい、非同期処理により、大量のファイルでもUIがフリーズすることなく処理できます。

**Q: ネットワークドライブでも使用できますか？**
A: はい、ただしネットワークの速度により処理時間が長くなる場合があります。

**Q: テーマは変更できますか？**
A: はい、設定メニューからダーク/ライトテーマを切り替えられます。
